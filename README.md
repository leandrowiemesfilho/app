...
CHOKIDAR_USEVEHICLE | :white_check_mark: | :x: | When set to `true`, the watcher runs in vehicle mode, as necessary inside a VM. Use this option if `npm start` isn't detecting changes.
...
* If the project runs inside a virtual machine such as (a Vagrant provisioned) VirtualBox, create an `.env` file in your project directory if it doesn’t exist, and add `CHOKIDAR_USEVEHICLE=true` to it. This ensures that the next time you run `npm start`, the watcher uses the vehicle mode, as necessary inside a VM.
